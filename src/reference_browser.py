...
def update_android_components(ac_repo, rb_repo, author, debug):
    ...
    if compare_ac_versions(current_ac_version, latest_ac_nightly_version) >= 0:
        print(
            f"{ts()} No need to upgrade; Reference-Browser is on A-C {current_ac_version}"
        )
        return

    print(
        f"{ts()} We should upgrade Reference Browser to Android-Components {latest_ac_nightly_version}"
    )
    ...
    _update_ac_version(
        rb_repo, pr_branch_name, current_ac_version, latest_ac_nightly_version, author
    )

    print(f"{ts()} Creating pull request")
    pr = rb_repo.create_pull(
